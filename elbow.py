...
def control_motor(motor, encoder, target_position):
    if(target_position<0):
        motor.setTargetVelocity(-0.25)  # Set motor speed (from -1.0 to 1.0)
...
def main():
    ...
    try:
        target_positions = [-2921]  # Example target positions for each motor for fo rward give negative tsrget position
